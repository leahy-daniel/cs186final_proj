...
public class LockManager {
  ...
  private void handlePotentialDeadlock(Lock lock, long transNum, LockType lockType) {
    //TODO: Implement Me!!
    for (long l : lock.getOwners()) {
      if (l != transNum) {
        if (!this.waitsForGraph.edgeCausesCycle(transNum, l)) {
          if (lockType.equals(LockManager.LockType.EXCLUSIVE) || lock.getType().equals(LockManager.LockType.EXCLUSIVE)) {
            if (!this.waitsForGraph.containsNode(transNum)) {
              this.waitsForGraph.addNode(transNum);
            }
            if (!this.waitsForGraph.containsNode(l)) {
              this.waitsForGraph.addNode(l);
            }
            this.waitsForGraph.addEdge(transNum, l);
          }
        } else {
